...
exports.default = {
    ...
    bootstrap({ strapi }) {
        ...
        io.on('connection', async (socket) => {
            ...
            if (token) {
                try {
                    const response = await axios_1.default.get('http://localhost:1337/api/users/me', {
            ...
            socket.on('createEvent', async (eventData) => {
                try {
                    const createdEvent = await axios_1.default.post('http://localhost:1337/api/events', { data: eventData });
            ...
            socket.on('updateEvent', async (eventId, updatedData) => {
                try {
                    const updatedEvent = await axios_1.default.put(`http://localhost:1337/api/events/${eventId}`, { data: updatedData });
                    // Émettre un événement WebSocket pour notifier les autres clients
            ...
            socket.on('deleteEvent', async (documentId) => {
                try {
                    await axios_1.default.delete(`http://localhost:1337/api/events/${documentId}`);
                    // Émettre un événement WebSocket pour notifier les autres clients
